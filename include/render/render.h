...
struct Vertex
{
    ...
    static std::vector<Vertex> create(std::vector<nevk::Vertex> const& params)
    {
        std::vector<Vertex> ret(params.size());
        std::transform(params.begin(), params.end(), ret.begin(),
                       [](auto value) {
                           return Vertex{value.pos, value.color, value.ka, value.kd, value.ks, value.uv};
                       });
        return ret;
    }
};
